...
func ReflectError(v interface{}) error {
	return fmt.Errorf("type is %T, value is:%#v", v, v)
}
...
//中序输出到channel当中
func (tree *rbTree) OutPut(buff chan<- Info) {
	tree.root.midInputChannel(buff)
}



func (node *rbTNode) midInputChannel(buff chan<- Info) {
	if node == nil {
		return 
	}
	node.left.midInputChannel(buff)
	buff <- Info{node.k[4:], basic.Strval(node.v)}
	node.right.midInputChannel(buff)
}
